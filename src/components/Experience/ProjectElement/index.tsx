...
const ProjectElement: React.FC<ProjectElementProps> = ({
    ...
    const [animatedName, setAnimatedName] = useState(ProjectName);
    const [isAnimating, setIsAnimating] = useState<boolean>(false);
    const [width, setWidth] = useState(0);
    const { reducedMotion } = usePreferences();
    const { startAnimation, isInProcess } = useAnimation();
    const navigate = useNavigate();
    const formattedDate = new Date(ProjectCreationDay).toLocaleDateString("en-US", {
    ...
    const animateName = () => {
        if (!isAnimating) {
            ...
            setAnimatedName('');
            let i = 0;
            const animate = (step:any, currentAnimatedName:any) => {
    ...
    return (
        <AnimationOnScroll animateIn="animate__fadeIn project-element-wrapper" animateOnce={true}>
            <button onClick={handleClick} className="project-element" onMouseEnter={animateName}>
                ...
                <div
                    className="project-techs">{ProjectTechnologies && ProjectTechnologies.length > 0 ? ProjectTechnologies.map((tech: string, key) => (
                    <span key={key}
                          className={`project-tech ${tech}`}>{tech}</span>)) : '-'}</div>
                <div className="project-img">
                    <i className="icon img"></i>
                    <span>{ProjectImages ? JSON.parse(ProjectImages[0]).length : 0}</span>
                </div>
